...
class WholeChinaService:
    ...
    def draw(self):
        ...
        baseboardHandler = ResourceBaseboardHandler(
            workstation=workstation, input_data=input_data, color_levels=color_levels,
            param_dict=china_params_dict.get('china_baseboard'))
        mapPlot = baseboardHandler.contour_map()
        workstationHandler.plot = mapPlot

        # 3.边界线绘制（可选）
        if True:
            ploylineHandler = ResourcePolylineHandler(workstation=workstation, plot=mapPlot,
                                                      param_dict=china_params_dict.get('china_geoline'))
            ploylineHandler.contour_map()

        # 4.添加中国图例
        if True:
            labelBarHandler = ResourceLabelBarHandler(workstation=workstation, plot=mapPlot
                                                      , output_img_name=output_img_name
                                                      , params_dict=china_params_dict.get('china_labelbar')
                                                      , location_params_dict=china_params_dict.get(
                    'china_labelbar_location')
                                                      , color_levels=color_levels, color_lables=color_lables)
            labelBarHandler.contour_map()

        # 5.添加中国比例尺
        if True:
            textHandler = ResourceTextHandler(workstation=workstation, plot=mapPlot,
                                              params_dict=china_params_dict.get('china_scale'))
            textHandler.contour_map()

        # 6.南海相关
        if True:
            southseaHandler = ResourceSouthSeaHandler(workstation=workstation, plot=mapPlot
                                                      , input_data=input_data, color_levels=color_levels)
        ...
        ResourceTitleAndLogoHandler.add_title(output_img_file=output_img_path + output_img_name + output_img_type
                                              ...
                                              , params_dict=china_params_dict.get('logo_and_titles'), standard='cn')
